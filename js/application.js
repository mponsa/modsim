...
function drawCard() {
  ...
  elem.append(
    `<div class="title" id="card-title">Aproximación númerica de ecuaciones diferenciales</div>`
  );
...
function appendEquation(elem, args) {
  ...
  elem.append(
    '<textarea class="equation-input" id="equation-input" name="equation-input" rows="1">2t</textarea>'
  );
}
...
function odeEuler(f, x0, t0, h, limit) {
  ...
  for (let i = 0; i < xRange.length - 1; i++) {
    ...
    xRange[i + 1] =
      xRange[i] + math.evaluate(f, { x: xRange[i], t: tRange[i] }) * h;
  }
  ...
  var g = board.create("curve", [tRange, xRange], {
    dash: 3,
    strokeColor: "#429615",
  });
}
...
function odeEulerImproved(f, x0, t0, h, limit) {
  ...
  for (let i = 0; i < xRange.length - 1; i++) {
    ...
    xRange[i + 1] =
      xRange[i] +
      (math.evaluate(f, { x: xRange[i], t: tRange[i] }) +
        math.evaluate(f, { xPred, t: tRange[i + 1] })) *
        (h / 2);
  }
...
function odeRungeKutta(f, x0, t0, h, limit) {
  ...
  for (let i = 0; i < xRange.length - 1; i++) {
    ...
    gamma2 = math.evaluate(f, {
      x: xRange[i] + 0.5 * h * gamma1,
      t: tRange[i] + 0.5 * h,
    });
    gamma3 = math.evaluate(f, {
      x: xRange[i] + 0.5 * h * gamma2,
      t: tRange[i] + 0.5 * h,
    });
    gamma4 = math.evaluate(f, { x: xRange[i] + gamma3 * h, t: tRange[i] + h });
    xRange[i + 1] =
      xRange[i] + (gamma1 + 2 * gamma2 + 2 * gamma3 + gamma4) * (h / 6);
  }
...
function appendGraph(elem, args) {
  ...
  const resize = function () {
    board.resizeContainer(
      board.containerObj.clientWidth,
...
function appendOriginalFuncInput(elem, args) {
  elem.append(
    '<label for="original-equation-input">Función original: </label>'
  );
  elem.append(
    '<textarea class="equation-input" id="original-equation-input" name="original-equation-input" rows="1"></textarea>'
  );
}
